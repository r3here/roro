...
const App: React.FC = () => {
  ...
  const handleBatchAiAnalyze = async () => {
    ...
    while (queue.length > 0) {
      ...
      // Dynamic delay: faster if more keys. 
      // 1 key = 3000ms, 2 keys = 1500ms, 6 keys = 500ms (min)
      ...
      try {
        ...
        console.error(`Analysis failed for ${item.title}`);
      ...
      processed++;
      setProgress({ current: Math.min(processed, targetItems.length), total: targetItems.length, label: "AI 整理中..." });
    }
